    ...
    <div className="min-h-full">
      ...
      <div className={`relative mb-6 overflow-hidden rounded-lg ${isDark ? "bg-vhs-card border-royalblue/20" : "bg-white/80 border-[#a89888]/30"} border`}>
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-5 p-4 sm:p-5">
          <div className={`shrink-0 h-24 w-24 sm:h-28 sm:w-28 rounded-lg border flex items-center justify-center ${isDark ? "border-royalblue/20 bg-royalblue/10" : "border-[#a89888]/40 bg-[#f5f0e8]"}`}>
            {playlist.isSystem
          ...
          <div className="space-y-1">
            {(playlist.tracks ?? []).map((pt, i) => {
              ...
              return (
                <div
                  key={track.id}
                  onClick={() => playTrack(track, tracks, "user_playlist")}
                ...
                >
                  ...
                  {canManageTracks && !playlist.isSystem && (
                    <button
                      ...
                      className={`shrink-0 rounded p-2 transition-all ${
                        removing === track.id
