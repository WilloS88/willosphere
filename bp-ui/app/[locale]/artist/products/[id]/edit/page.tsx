...
function EditProductContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get<ProductDto>(API_ENDPOINTS.products.detail(Number(id))),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.patch(API_ENDPOINTS.products.detail(Number(id)), {
        name:        form.name.trim(),
        type:        form.type,
          ...
          {loading ? (
            ...
            <form onSubmit={(e) => void handleSubmit(e)} className="space-y-5">
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  {saving ? t("creating") : t("productSaved")}
                </button>
              </div>
            </form>
          )}
